...
@else
    {{-- Full lockup: COMILOG Local Connect (text hidden on xs) --}}
    <div {{ $attributes->merge(['class' => 'flex items-center gap-2 sm:gap-3']) }}>
        <img src="{{ asset('images/comilog-logo.png') }}" alt="ERAMET COMILOG" class="h-8 sm:h-9 w-auto">
        <span class="hidden sm:inline-block h-7 w-px bg-stone-300/80"></span>
